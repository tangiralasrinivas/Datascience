...
namespace WebAutomation.Support.Core
{
    ...
    public partial class PageBase
    {
        ...
        public PageBase(List<PageContext> context)
        {
            foreach (var contxt in context)
            {
                this.Context = contxt;
                PageFactory.InitElements(this.Context.Driver, this);
            }
        ...
        {
            WaitForVisibility(By.Id(id), 300).Click();
        }

        public void ClickByIdAndWait(string id, int waitTime)
        {
            WaitForVisibility(By.Id(id), waitTime).Click();
        }
